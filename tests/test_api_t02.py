...
def test_health_exposes_t02_edge_and_read_only_contract():
    result = health()
    assert result["version"] == "0.3.1"
    assert result["fusion_version"] == "t02-edge-v1"
    assert result["model_id"] == "flowtrust-linear-prior-v1"
    ...
    for label in CLASSES:
        for _ in range(8):
            ...
            if result["fusion"] is not None:
                assert result["fusion"]["version"] == "t02-edge-v1"
...
def test_api_never_forces_diagnosis_when_observability_fails():
    ...
    features["electrical_signal_quality"] = 0.10
...
def test_api_missing_fraction_over_limit_abstains_before_inference():
...
def test_camera_loss_does_not_force_abstention_when_two_other_modalities_are_reliable():
    features = make_synthetic_sample("normal", np.random.default_rng(101))
    features["camera_connected"] = 0.0
    features["camera_quality"] = 0.0

    result = diagnose(Payload(features=features))

    assert result["automatic_control_allowed"] is False
    assert result["abstained"] is False
    assert result["diagnostic"] == "normal"
    assert result["fusion"]["modalities"]["vision"]["active"] is False
